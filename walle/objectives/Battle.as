...
class walle.objectives.AutoDigger implements Objective {
	...
	public function AutoDigger(settings : Settings) {
		_settings = settings;
	}
	...
	private function getBoxes() {
		...
		droped = ArrayUtilites.filter(droped, function(item : Object) {
			printDebug(item.txt);
			var name : String = "walle.pickup." + item.txt.toLowerCase().split(' ').join('_');
			
			if (_settings.get(name) == undefined || _settings.get(name) == true) {
				printDebug("OK");
				return true;
			} else {
				return false;
	...
	public function onTimer() : Void {
		if(Battle.inBattle) {
			...
			processBattle2();
		} else if (_settings.get("walle.autobattle")) {
			if(User.inBattle) {
				Battle.exit();
